...
export async function generateVerificationToken(email: string, minute?: number) : Promise<{ status: number, data: any }> {
    const e=await getTranslations('Error');
    try {
        const token=generateToken4Chiffres();
        const expiredAt=new Date(new Date().getTime() + (minute??60)*60*1000);
