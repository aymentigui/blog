...
import { AddProjectsCategory } from "@/actions/project/categories/set";
import { UpdateProjectsCategory } from "@/actions/project/categories/update";
...
export const AddUpdateProjectsCategoryDialog = () => {
  const translate = useTranslations("ProjectsCategories");
  const { isOpen, closeDialog, isAdd, data } = useAddUpdateProjectsCategoryDialog();
  ...
  const onSubmit = async (dataFrom: FormValues) => {
    ...
    if (isAdd) {
      res = await AddProjectsCategory(dataFrom.name,dataFrom.namefr??"",dataFrom.namear??"");
    } else if(data) {
      res = await UpdateProjectsCategory(data.id,dataFrom.name,dataFrom.namefr??"",dataFrom.namear??"");
    } else {
      toast.error("Error");
      return
    }
